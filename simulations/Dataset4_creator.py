...
def preview_results(mask_skull, source_mask, p_max_norm, i, dx, Nx, Ny, Nz, is_water_only):
    ...
    for j in range(3):
        ax[0, j].imshow(slices_geo[j], extent=ext, origin="lower", cmap="gray", vmin=0, vmax=1)
    ...
    estado = "(SOLO AGUA)" if is_water_only else "(CON CRÁNEO)"
    plt.suptitle(f"Simulación {i:04d} {estado} | dx={dx*1e3}mm | f={400}kHz", fontsize=14)
    plt.tight_layout()
    plt.show()
...
def main():
    OUT_DIR = "dataset_TUS_dx10_f400_R60"
    os.makedirs(OUT_DIR, exist_ok=True)


    N_SIMS = 1000
    N_WATER_ONLY = 100  # Casos de control (baseline)
    SEED = 123
    rng = np.random.default_rng(SEED)


    # Crear vector de banderas aleatorias para decidir cuáles son agua y cuáles cráneo
    water_flags = np.array([True]*N_WATER_ONLY + [False]*(N_SIMS - N_WATER_ONLY))
    rng.shuffle(water_flags)
    ...
    F0 = 400e3  # 400 kHz como discutimos (mejor física)
    N_CYCLES = 10
    R_CONST = 60e-3
    APERTURE = 50e-3
    r_T = 50e-3
    ...
    TMIN, TMAX = 3.0e-3, 7.0e-3  # Grosor realista ajustado
    ...
    props = {
        ...
        "skull": (2800.0, 1850.0, 15.0), # Standard
    ...
    print(f"📡 Iniciando Dataset: {N_SIMS} sims ({N_WATER_ONLY} control) | f={F0/1e3}kHz | dx={dx*1e3}mm")


    for i in range(N_SIMS):
        is_water_only = water_flags[i]


        # 1. Geometría
        if is_water_only:
            # Matrices vacías (todo False)
            m_skull = np.zeros((Nx, Ny, Nz), dtype=bool)
            m_brain = np.zeros((Nx, Ny, Nz), dtype=bool)
            a = b = c = t_mean = gap_real = 0.0
        else:
            # Cráneo aleatorio
            a, b, c = sample_skull_axes_for_fixed_rT_gap_ovoid(rng, r_T, GAP_MIN, GAP_MAX)
            m_skull, m_brain, _, t_mean = create_skull_masks_procedural_TAC(kgrid, a, b, c, TMIN, TMAX, rng)
        ...
        if n_pts == 0: 
            print(f"⚠️ Sim {i} saltada: El transductor quedó en mal lugar.")
            continue
        ...
        # Asignación del medio dinámica (si no hay cráneo/cerebro, todo será agua por defecto)
        medium = kWaveMedium(
        ...
        # 4. Ejecutar
        tipo_str = "[AGUA]" if is_water_only else "[CRÁNEO]"
        print(f"\n>> Sim {i:04d}/{N_SIMS-1} {tipo_str} | P_pts: {n_pts}")
        ...
        np.savez_compressed(
            out_path,
            is_water_only=np.bool_(is_water_only),
        ...
        # Mostrar preview solo de los primeros 3 casos para asegurar que todo va bien
        if i < 3: 
            preview_results(m_skull, s_mask_u8, p_max_norm, i, dx, Nx, Ny, Nz, is_water_only)
