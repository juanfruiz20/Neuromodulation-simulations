...
def approx_max(x: torch.Tensor, mask: torch.Tensor = None, beta: float = 20.0):
    """
    Aproximaci�n diferenciable del m�ximo usando logsumexp estable.
...
def soft_argmax3d(x: torch.Tensor, mask: torch.Tensor = None, beta: float = 20.0):
    """
    Devuelve coordenadas normalizadas en [-1,1] del centro del m�ximo.
    x: [B, 1, D, H, W]
    """
    x = x.float()
    B, C, D, H, W = x.shape
...
class StableFocusAwareTUSLoss_ExpB(nn.Module):
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor, epoch: int = 1, return_dict: bool = False):
        ...
        # -------------------------
        # 5) Peak location loss (ROI estrecha)
        # -------------------------
...
def main():
    ...
    SAVE_DIR = "checkpoints_unet_V2_expB"
    ...
    criterion = StableFocusAwareTUSLoss_ExpB(
        lambda_global=0.8,
        lambda_focus=2.0,
        lambda_peak=0.8,
        lambda_loc=0.6,
