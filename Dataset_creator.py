"""
This script generates a dataset of 3D ultrasound neuromodulation simulations:
- Transducer: fixed bowl-shaped oriented towards the center, 
    with specified aperture, curvature radius, and thickness.
- Skull: procedural TAC-like ovoid geometry with guaranteed gap to transducer, 
    variable thickness, and smooth anisotropic + angular variations.
- Medium: 3D maps of sound speed, density, and absorption for water, skull, and brain.
- Signal: tone burst with specified frequency, cycles, and amplitude.
The script saves each simulation as a compressed .npz file containing:
- Input parameters: skull masks, medium properties, transducer geometry, signal params.
- Output: 3D map of maximum pressure (p_max) normalized by source amplitude.
The script also includes a preview of the geometry for the first 2 simulations.
Key features:
- Robust time array handling with fallback if makeTime is not compatible.
- Safe reshaping of p_max output.
- Configurable parameters for transducer, skull, signal, and medium properties.
Usage:
- Install dependencies: kwave, numpy, matplotlib.
"""
