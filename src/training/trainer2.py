...
# =========================================================
# Loss flexible para baseline y ablations
# =========================================================
class FlexibleAblationTUSLoss(nn.Module):
    """
    Loss modular para comparar baseline y ablations.
    ...
    Ablations posteriores:
        activar progresivamente lambda_focus, lambda_grad,
        lambda_tube, lambda_profile.
    """
    ...
    def forward(self,
        ...
        # -------------------------------------------------
        # Peak y ROI focal solo se calculan si hacen falta
        # -------------------------------------------------
        need_focus_roi = (
            self.lambda_focus > 0.0
            or self.lambda_grad > 0.0
            or self.lambda_tube > 0.0
            or self.lambda_profile > 0.0
            or self.global_peak_weight > 0.0
        )
        ...
        if need_focus_roi:
            ...
            peak = target32.amax(dim=(2, 3, 4), keepdim=True).clamp_min(1e-6)
            focus_roi = None

        # -------------------------------------------------
        # 1) Global L1 o weighted global L1
        # -------------------------------------------------
        if self.global_peak_weight > 0.0:
            ...
            loss_global = F.l1_loss(pred_pos, target32)

        # -------------------------------------------------
        # 2) Focus relative MSE
        # -------------------------------------------------
        if self.lambda_focus > 0.0:
            ...
            loss_focus = torch.zeros((), device=pred.device, dtype=torch.float32)

        # -------------------------------------------------
        # 3) Gradient loss en ROI focal
        # -------------------------------------------------
        if self.lambda_grad > 0.0:
            ...
            loss_grad = torch.zeros((), device=pred.device, dtype=torch.float32)

        # -------------------------------------------------
        # 4) Tube ROI
        # -------------------------------------------------
        if sched["lambda_tube_eff"] > 0.0 or sched["lambda_profile_eff"] > 0.0:
            if source_mask is None:
                raise ValueError("source_mask es necesario para tube/profile losses.")
...
def train_one_epoch(G, D,
    ...
    for step, (X, y) in enumerate(loader, start=1):
        ...
        if update_D:
            ...
            if not torch.isfinite(loss_d):
                print(f"[WARN] Non-finite loss_D en train, epoch {epoch}, step {step}. Batch saltado.")
                continue
        ...
        if not torch.isfinite(loss_g):
            print(f"[WARN] Non-finite loss_G en train, epoch {epoch}, step {step}. Batch saltado.")
            continue
        ...
        g_recon += float(loss_recon.item())
        g_adv += float(loss_adv_g.item())
        d_total += d_total_item
        d_real_mean += d_real_item
        d_fake_mean += d_fake_item
        n_batches += 1
    ...
    if n_batches == 0:
        return {
            ...
            "g_adv": float("inf"),
            "d_total": float("inf"),
            "d_real_mean": float("inf"),
            "d_fake_mean": float("inf"),
        }

    return {
        "g_total": g_total / n_batches,
        "g_recon": g_recon / n_batches,
        "g_adv": g_adv / n_batches,
        "d_total": d_total / n_batches,
        "d_real_mean": d_real_mean / n_batches,
        "d_fake_mean": d_fake_mean / n_batches,
    }
...
def eval_one_epoch(G, D,
    ...
    for X, y in loader:
        ...
        with amp_ctx:
            ...
            loss_recon, comp = recon_criterion(
                y_hat,
                y,
                source_mask=source_mask,
                epoch=epoch,
        ...
        if (not torch.isfinite(loss_g)) or (not torch.isfinite(loss_d)):
            print(f"[WARN] Non-finite loss en val, epoch {epoch}. Batch saltado.")
            continue
        ...
        d_real_mean += float(pred_real.detach().mean().item())
        d_fake_mean += float(pred_fake.detach().mean().item())
        n_batches += 1
...
def append_csv(csv_path, epoch, train_stats, val_stats, val_comp, extra_val_stats, lambda_adv, lr_G, lr_D, time_sec):
    def _pick(d, k):
        if d is None:
            return float("nan")
...
def log_epoch_to_tensorboard(writer: SummaryWriter,
    ...
    writer.add_scalar("recon/val_global", val_comp["loss_global"], epoch)
    writer.add_scalar("recon/val_focus", val_comp["loss_focus"], epoch)
    writer.add_scalar("recon/val_grad", val_comp["loss_grad"], epoch)
    writer.add_scalar("recon/val_tube", val_comp["loss_tube"], epoch)
    writer.add_scalar("recon/val_profile", val_comp["loss_profile"], epoch)
...
def log_extra_metrics_to_tensorboard(writer: SummaryWriter, epoch: int, stats: Dict[str, float], split: str = "val_extra"):
...
def main():
    ...
    # Cambia este nombre para cada ablation.
    SAVE_DIR = "checkpoints_cgan_base01_globalL1"
    ...
    SPECIFIC_SAVE_EPOCHS = {50, 100, 150, 200, 250, 300}
    ...
    # ABLATION CONFIG
    # =====================================================
    # Baseline 1: Global L1 + adversarial
    # Para las siguientes ablations, cambia solo estos valores.
    LOSS_CFG = {
        "lambda_global": 1.0,
        "lambda_focus": 0.0,
        "lambda_grad": 0.0,
    ...
    # Ejemplos de ablations posteriores:
    #
    # Ablation 2: Global L1 + Focus + adversarial
    # LOSS_CFG["lambda_focus"] = 1.5
    #
    # Ablation 3: Global L1 + Focus + Grad + adversarial
    # LOSS_CFG["lambda_focus"] = 1.5
    # LOSS_CFG["lambda_grad"] = 0.08
    #
    # Ablation 4: Global weighted + Focus + Grad + adversarial
    # LOSS_CFG["lambda_focus"] = 1.5
    # LOSS_CFG["lambda_grad"] = 0.08
    # LOSS_CFG["global_peak_weight"] = 4.0
    #
    # Ablation 5: Modelo completo TubeAware
    # LOSS_CFG["lambda_focus"] = 1.5
    # LOSS_CFG["lambda_grad"] = 0.08
    # LOSS_CFG["lambda_tube"] = 1.0
    # LOSS_CFG["lambda_profile"] = 0.6
    # LOSS_CFG["global_peak_weight"] = 4.0
    ...
    # LOSSES
    # =========================
    recon_criterion = FlexibleAblationTUSLoss(
        lambda_global=LOSS_CFG["lambda_global"],
        lambda_focus=LOSS_CFG["lambda_focus"],
        lambda_grad=LOSS_CFG["lambda_grad"],
        lambda_tube=LOSS_CFG["lambda_tube"],
        lambda_profile=LOSS_CFG["lambda_profile"],

        focus_frac=LOSS_CFG["focus_frac"],
        focus_min_thr=LOSS_CFG["focus_min_thr"],
        focus_dilate_ks=LOSS_CFG["focus_dilate_ks"],

        tube_radius_vox=LOSS_CFG["tube_radius_vox"],
        profile_bins=LOSS_CFG["profile_bins"],

        global_peak_weight=LOSS_CFG["global_peak_weight"],
        global_peak_gamma=2.0,

        tube_warmup_start=LOSS_CFG["tube_warmup_start"],
        tube_warmup_end=LOSS_CFG["tube_warmup_end"],
        profile_warmup_start=LOSS_CFG["profile_warmup_start"],
        profile_warmup_end=LOSS_CFG["profile_warmup_end"],
    ...
    config = {
        ...
        "LOSS_CFG": LOSS_CFG,
        "EXPERIMENT": "baseline_1_global_L1_plus_adversarial",
    }
    ...
    try:
        for epoch in range(1, EPOCHS + 1):
            ...
            va_stats, va_comp = eval_one_epoch(
                G, D,
                val_loader,
                recon_criterion, adv_criterion,
                device,
            ...
            print(
                f"Epoch {epoch:03d} | "
                f"trainG={tr['g_total']:.6f} | "
                f"trainRecon={tr['g_recon']:.6f} | "
                f"trainAdv={tr['g_adv']:.6f} | "
                f"trainD={tr['d_total']:.6f} | "
                f"valG={va_stats['g_total']:.6f} | "
                f"valRecon={va_stats['g_recon']:.6f} | "
                f"valAdv={va_stats['g_adv']:.6f} | "
                f"valD={va_stats['d_total']:.6f} | "
            ...
            if va_stats["g_total"] < best_val:
                best_val = va_stats["g_total"]
                save_ckpt(
