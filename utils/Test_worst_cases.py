...
# Ajusta esta importaci�n solo si en tu proyecto la ruta es distinta
...
CKPT_PATH = r"/data/home/agustin/Documents/oslo/TFG Juanfe/Neuromodulation-simulations/checkpoints_cgan_exp01_fromscratch_tb/epoch_100.pth"

OUT_DIR = r"/data/home/agustin/Documents/oslo/TFG Juanfe/Neuromodulation-simulations/tablas/worstFile/worst3"

# =========================
# SOLO PON AQU� EL CSV
# =========================
CSV_PER_SAMPLE_PATH = r"/data/home/agustin/Documents/oslo/TFG Juanfe/Neuromodulation-simulations/checkpoints_unet_expDexpB/test_metrics/epoch_030_per_sample.csv"
...
# N�mero de peores casos a visualizar
N_WORST_CASES = 3

# C�mo seleccionar los peores casos:
# - "selection_score" -> score compuesto reconstruido desde el CSV
# - o una columna directa del CSV, por ejemplo:
#   "peak_loc_err_mm_brain"
#   "peak_rel_err_brain"
#   "mae_brain"
#   "mse_brain"
#   "profile_mae_centroid_to_end"
WORST_METRIC = "selection_score"

# Si quieres forzar casos concretos, ponlos aqu�.
# Si FORCE_FILES no est� vac�o, ignora el CSV.
FORCE_FILES = set()
# Ejemplo:
...
# =========================================================
# SELECT WORST CASES FROM CSV
# =========================================================
def select_worst_cases_from_csv(csv_path: str,
                                n_cases: int = 5,
                                metric: str = "selection_score"):
    """
    Lee el CSV per-sample y selecciona los peores casos.

    Si metric == "selection_score", reconstruye un score compuesto:
        0.45 * peak_loc_err_mm_brain
      + 0.25 * peak_rel_err_brain
      + 0.20 * mae_brain
      - 0.05 * dice_focus_brain_thr50
      - 0.05 * ssim_brain

    Si metric es otra columna existente del CSV, ordena por esa columna.
    M�s alto = peor.
    """
    rows = []
    ...
    with open(csv_path, "r", encoding="utf-8") as f:
        ...
        if cols is None:
            raise RuntimeError(f"CSV vac�o o inv�lido: {csv_path}")

        if "file" not in cols:
            raise RuntimeError(
                f"El CSV no contiene la columna 'file'. Columnas disponibles: {cols}"
            )

        if "is_water_only" not in cols:
            raise RuntimeError(
                f"El CSV no contiene la columna 'is_water_only'. Columnas disponibles: {cols}"
            )

        for row in reader:
            try:
                is_water_only = int(float(row["is_water_only"]))
                if is_water_only == 1:
                    continue
                ...
                if metric == "selection_score":
                    ...
                    for k in needed:
                        if k not in cols:
                            raise RuntimeError(
                                f"Falta la columna '{k}' para construir selection_score"
                            )

                    peak_loc = float(row["peak_loc_err_mm_brain"])
                    peak_rel = float(row["peak_rel_err_brain"])
                    mae_brain = float(row["mae_brain"])
                    ssim_brain = float(row["ssim_brain"])
                    dice50 = float(row["dice_focus_brain_thr50"])

                    if not np.isfinite(peak_loc):
                        peak_loc = 1e6
                    if not np.isfinite(peak_rel):
                        peak_rel = 1e6
                    if not np.isfinite(mae_brain):
                        mae_brain = 1e6
                    if not np.isfinite(ssim_brain):
                        ssim_brain = 0.0
                    if not np.isfinite(dice50):
                        dice50 = 0.0
                    ...
                    rows.append({
                        ...
                        "peak_loc_err_mm_brain": peak_loc,
                        "peak_rel_err_brain": peak_rel,
                        "mae_brain": mae_brain,
                        "ssim_brain": ssim_brain,
                        "dice_focus_brain_thr50": dice50,
                    })

                else:
                    if metric not in cols:
                        raise RuntimeError(
                            f"La m�trica '{metric}' no est� en el CSV.\n"
    ...
    if len(rows) == 0:
        raise RuntimeError(f"No se pudieron leer casos v�lidos desde: {csv_path}")

    rows_sorted = sorted(rows, key=lambda r: r["metric_value"], reverse=True)
    worst_rows = rows_sorted[:n_cases]
    worst_files = {r["file_name"] for r in worst_rows}

    print(f"\nPeores casos desde CSV seg�n '{metric}':")
    for i, r in enumerate(worst_rows, 1):
        if metric == "selection_score":
            print(
                f"{i:02d}. {r['file_name']} | "
                f"score={r['metric_value']:.6f} | "
                f"peak_loc_mm={r['peak_loc_err_mm_brain']:.4f} | "
                f"peak_rel={r['peak_rel_err_brain']:.4f} | "
                f"mae_brain={r['mae_brain']:.6f} | "
                f"dice50={r['dice_focus_brain_thr50']:.4f} | "
                f"ssim_brain={r['ssim_brain']:.4f}"
            )
        else:
            print(f"{i:02d}. {r['file_name']} | {metric}={r['metric_value']:.6f}")

    return worst_files, worst_rows


def save_selected_cases_csv(rows, csv_path):
    if len(rows) == 0:
        return
...
def main():
    ...
    print("CSV_PER_SAMPLE_PATH:", CSV_PER_SAMPLE_PATH)
    print("DX_MM:", DX_MM)
    ...
    # 2) Validar FORCE_FILES
    validate_force_files(test_ds, FORCE_FILES)

    # 3) Cargar modelo/generator
    model = load_model(CKPT_PATH, DEVICE)

    # 4) Seleccionar casos a visualizar
    if FORCE_FILES:
        vis_files = set(FORCE_FILES)
        worst_rows = []
    else:
        vis_files, worst_rows = select_worst_cases_from_csv(
            csv_path=CSV_PER_SAMPLE_PATH,
            n_cases=N_WORST_CASES,
            metric=WORST_METRIC,
        )

        save_selected_cases_csv(
            worst_rows,
            os.path.join(OUT_DIR, "selected_worst_cases.csv")
        )
